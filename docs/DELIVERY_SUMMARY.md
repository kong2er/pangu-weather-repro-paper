...
| 阶段 | 目标 | 命令 | 关键产物 | 通过标准 |
|---|---|---|---|---|
| S1 | 创建 CPU/GPU 环境 | **前置：安装 uv**（`curl -LsSf https://astral.sh/uv/install.sh \| sh`）<br>`bash scripts/create_cpu_venv.sh`<br>`bash scripts/create_gpu_venv.sh`<br>`source scripts/env_gpu.sh` | `.venv-cpu` / `.venv-gpu` | `env_gpu.sh` 输出包含 `CUDAExecutionProvider` |
| S2 | CPU smoke | `scripts/run_cpu.sh -m pangu_weather_repro.smoke` | 终端 smoke 输出 | `contracts smoke ok` |
| S3 | GPU smoke | `bash scripts/prepare_era5_inputs.sh --yes`<br>`bash scripts/internal/run_day3_smoke_gpu.sh` | `smoke_24h_report.json` | Day3 smoke 报告生成 |
...
| S6 | RMSE + paper 图 | `bash scripts/internal/run_day5_rmse.sh --force`<br>`bash scripts/internal/run_day6_plots.sh --force` | `artifacts/day5/rmse.csv`<br>`figures/day6/*.png` | 文件存在且非空 |
...
## 快速排错
- uv 未安装：`curl -LsSf https://astral.sh/uv/install.sh | sh && source ~/.bashrc`
