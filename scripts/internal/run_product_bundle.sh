...
if [[ "${1:-}" == "--help" ]]; then
  echo "Usage: scripts/internal/run_product_bundle.sh [--force] [--with-geo] [--vars z500,t2m,u10] [--hours 24,30] [--kinds fill,diff,vector,msl_wind] [--impl auto|native|blueprint] [--rollout-dir PATH]"
  echo "中文说明: 生成产品图（figures/product/*.png + *.json），默认不覆盖。"
  echo "Example: scripts/internal/run_product_bundle.sh --vars z500,t2m --hours 24,30"
  exit 0
fi
